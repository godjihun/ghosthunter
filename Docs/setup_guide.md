...
⚠️ **메뉴 키로 `ESC`를 쓰면 안 된다.** 브라우저가 포인터 잠금 해제·전체화면 종료에
...
### 에디터 API 저장은 디스크에 안 쓰일 수 있다
`AssetDatabase.SaveAssets()`가 **아무 오류 없이 파일을 안 쓰는** 경우가 있다(FBX 임포트가 멈추는 것과 같은 계열).
그 상태에서 스크립트가 재컴파일되면 도메인 리로드로 **메모리의 변경까지 사라진다.**

**저장 확인을 에디터에 되물어서 하면 안 된다.** 메모리 객체를 다시 읽는 것이라 저장 증거가 못 된다 —
실제로 애니메이터 블렌드 임계값을 "저장 완료"로 판단했다가 통째로 잃었다.
`git status`나 파일 직접 읽기로 확인할 것. `.controller`·`.prefab`·`.asset`은 전부 YAML 텍스트라
정 안 되면 직접 편집하고 `ImportAsset`으로 다시 읽히면 된다.
...
- **최장축이 Y가 아니면** 누워 있는 것 (Z-up glb → `Visual`에 X축 `-90°`)
- **수평 중심(X, Z)이 0이 아니면** 회전 시 공전한다

**회전을 고쳤으면 배율·바닥 정렬·콜라이더를 전부 다시 잡는다.** 축이 바뀌었으니 이전에 잰 값은 전부 무효다. 순서는 ① 회전 보정 → ② 스케일 1로 되돌리고 다시 재서 목표 높이로 나눔 → ③ 아래로 레이캐스트해 밑면을 바닥에 붙임 → ④ 최종 바운즈로 콜라이더 재설정. 제단에서 이 순서를 지키지 않아 세우고 나서 공중에 떠 있었다.

바닥을 잴 때는 **자기 콜라이더를 잠깐 꺼야 한다** — 안 그러면 자기 자신에 맞고, 원래 배치 y값은 자리 표시용 큐브 기준이라 믿을 수 없다.
