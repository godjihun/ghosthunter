...
- **사용자가 테스트하는 동안 씬을 건드리지 말 것.** 진단한답시고 오브젝트를 꺼봤다가 사용자 화면에 가짜 증상을 만들어낸 적이 있다. `GameObject.Find()`는 비활성 오브젝트를 못 찾으므로 되돌리기 스크립트가 조용히 실패한다.
- **에셋을 고쳤으면 반드시 `git status`로 확인할 것.** `AssetDatabase.SaveAssets()`가 오류 없이 디스크에 안 쓰는 경우가 있고, 그 뒤 재컴파일이 일어나면 메모리의 변경까지 사라진다. **에디터에 값을 되물어 확인하면 안 된다** — 메모리 객체를 다시 읽는 것이라 저장 증거가 못 된다. 실제로 이걸로 애니메이터 수정을 통째로 잃고 "완료"라고 보고한 적이 있다. 자세한 대처는 setup_guide.md 4번.
